...
//*/
	...
	function svgStyle($critere_style, $attribs, $element)
	{
		...
		$prestyle = $path_style . ' ';
		$_POSTstyle = $w . ' ' . $op . $fr . ' ' . $fill_gradient . "\n";
		return [$prestyle, $_POSTstyle];
	}
	...
	function xml_svg2pdf_start($parser, $name, $attribs)
	{
		...
		if (isset($path_cmd) && $path_cmd) {
			// mPDF 5.0
			list($prestyle, $_POSTstyle) = $this->svgStyle($path_style, $attribs, strtolower($name));
			if (isset($path_style['transformations']) && $path_style['transformations']) { // transformation on an element
				$this->svgWriteString(" q " . $path_style['transformations'] . $prestyle . $path_cmd . $_POSTstyle . " Q\n");
			} else {
				$this->svgWriteString(" q " . $prestyle . $path_cmd . $_POSTstyle . " Q\n"); // mPDF 5.7.4
			}
		}
	}
